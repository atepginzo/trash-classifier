  ...
  <>
    {CATEGORIES.map((label, i) => (
      <span key={i} className="flex items-center gap-0 shrink-0">
        <span className="text-sm sm:text-base font-medium text-heading/60 whitespace-nowrap tracking-wide">
          {label}
        </span>
        <span className="text-forest/30 mx-4 sm:mx-6 text-lg">·</span>
      </span>
    ))}
  </>
);
...
export default function Marquee() {
  return (
    <section className="relative w-full overflow-hidden py-5 sm:py-6 border-y border-sage/20 bg-cream-light/50">
